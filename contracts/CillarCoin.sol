...
uint256 public cillarUsdPrice = 10 * 10 ** 16; // $0.10 with 18 decimals
...
function ridePayment(bytes32 rideId, uint256 region, uint256 zone) external onlyKYCed {
    require(!rides[rideId].confirmed, "Ride already paid");

    IFareRegistry registry = IFareRegistry(fareRegistry);
    uint256 nadFare = registry.getFare(region, zone);
    uint256 usdFare = nadFare * 10 ** 18 / registry.getExchangeRate();
...
interface IFareRegistry { function getFare(uint256 region, uint256 zone) external view returns (uint256); function getExchangeRate() external view returns (uint256); }
